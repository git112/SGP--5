...
const ExperienceItem = ({ item, theme }) => {
  return (
    <div className="mb-4">
      ...
      {item.organization && (
        <p className="text-sm text-slate-700 font-medium mb-1">{item.organization}</p>
      )}
...
const CertificateItem = ({ cert, theme }) => (
  <div className="flex justify-between items-center mb-1">
    ...
    <div className="flex items-center">
      <span className="text-xs text-slate-500 mr-2">{cert.date}</span>
      {cert.link && (
        <a
          href={cert.link}
          target="_blank"
...
const ModernResumeTemplate = ({
  ...
  return (
    <div className="bg-white text-slate-800 shadow-lg max-w-4xl mx-auto font-sans border border-slate-200 print:border-0 print:shadow-none">
      {/* Header Section */}
      <header className="border-b border-slate-200 p-6">
        <h1 className="text-2xl font-bold text-slate-900 uppercase text-center">{name}</h1>

        {/* Contact Information Row */}
        <div className="flex flex-wrap justify-between text-xs mt-2">
          {contactInfo.email && (
            <a href={`mailto:${contactInfo.email}`} className="text-slate-700 hover:underline">
              Email: {contactInfo.email}
            </a>
          )}
          {contactInfo.mobile && (
            <a href={`tel:${contactInfo.mobile}`} className="text-slate-700 hover:underline">
              Mobile: {contactInfo.mobile}
            </a>
          )}
          {contactInfo.linkedin && (
            <a href={`https://linkedin.com/in/${contactInfo.linkedin}`} className="text-slate-700 hover:underline">
              LinkedIn: {contactInfo.linkedin}
            </a>
          )}
          {contactInfo.website && (
            <a href={contactInfo.website} className="text-slate-700 hover:underline">
