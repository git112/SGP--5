...
export const PlacementChatbot: React.FC<ChatbotProps> = ({ className }) => {
  ...
  const renderMessageContent = (text: string, isUser: boolean) => {
    return (
      <div className="wmde-markdown-var" data-color-mode={isUser ? "light" : "dark"}>
        <MDEditor.Markdown
          source={text}
          style={{
            whiteSpace: 'pre-wrap',
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ message: currentQuery, stream: true })
      });
          ...
          for (const line of lines) {
            ...
            if (data.startsWith('[SOURCES]')) {
              try {
                ...
                console.error("Error parsing sources", e); // Changed print to console.error
              }
              continue;
            }
            if (data) {
      ...
      setMessages(prev => prev.map(msg =>
        msg.id === botMessageId ? { ...msg, text: 'Sorry, something went wrong. Please try again.' } : msg
      ));
    } finally {
      setIsLoading(false);
      setIsTyping(false);
      ...
      <div className={`fixed bottom-4 right-4 z-50 ${className || ''}`}>
        ...
        {isOpen && (
          <div className="transform transition-all duration-300 ease-in-out relative z-50">
            <Card className={`w-80 sm:w-96 shadow-2xl border-2 border-primary/20 backdrop-blur-sm bg-background/95 transition-all duration-300 ${isMinimized ? 'h-16' : 'h-[400px] sm:h-[500px]'
              }`}>
                  ...
                  <ScrollArea className="flex-1 px-4 py-2" ref={scrollAreaRef}>
                    <div className="space-y-4">
                      {messages.map((message, index) => (
                        ...
                        >
                          <div
                            className={`max-w-[85%] sm:max-w-[80%] rounded-2xl px-4 py-3 shadow-md transition-all duration-200 hover:shadow-lg ${message.sender === 'user'
                          ...
                          >
                            <div className="flex items-start gap-2">
                              <div className="flex-1 min-w-0">
                                <div className="text-sm leading-relaxed min-h-[1.25rem] flex items-center">
                                  {message.text ? (
