...
Remember: Your job is to give them the harsh truth they need to hear, not to make them feel good about poor performance."""
    ...
    try:
        ...
        for ln in lines:
            ...
            if ln.lower().startswith("- feedback:"):
                feedback = ln.split(":", 1)[1].strip() or feedback
        if score < 0 or score > 100:
            raise ValueError("Invalid score generated by model")
        return { "score": score, "feedback": feedback }
    except Exception:
        # Fallback brutally honest feedback
        if answer_length < 30:
            return {
                "score": 5,
                "feedback": "BRUTALLY HONEST: This answer is so poor it's almost comical. You've demonstrated zero professional communication skills and appear completely unprepared for any serious role. This level of performance suggests you need to completely restart your career preparation from scratch."
            }
        return { "score": 25, "feedback": "Analysis failed, but based on answer length, this appears to be a poor response that needs significant improvement." }
